const SUPABASE_URL='https://xjngdyhzpgktddxfmdgq.supabase.co';
const SUPABASE_KEY='sb_publishable_gS5lC6Jhugq73v-_MMzM7g_GGWFDNZc';
...
const KEY='andressa_anamnese_v6';
...
function toggle(){
  document.getElementById('dor-extra').classList.toggle('show',val('dor')==='Sim');
  document.getElementById('med-extra').classList.toggle('show',val('medica')==='Sim');
}
...
function progress(){
  ...
  sections.forEach(sec=>{
    ...
    if(controls.some(c=>c.type==='radio'||c.type==='checkbox'?c.checked:(c.value||'').trim()))done++;
  });
  const n=Math.round(done/sections.length*100);
  fill.style.width=n+'%';
  pct.textContent=n+'%';
}
...
function load(){
  ...
  if(!raw){toggle();progress();return;}
  try{
    const data=JSON.parse(raw);
    fields.forEach(f=>{
      if(f.type==='radio')f.checked=data[f.name]===f.value;
      else if(f.type==='checkbox')f.checked=(data[f.name]||[]).includes(f.value);
      else if(data[f.name]!==undefined)f.value=data[f.name];
    });
  }catch(e){}
  toggle();
  progress();
...
function buildPayload(){
  return {
    ...
    mudanca: val('mudanca') || null,
    regioes: checked('regioes'),
    prioridade_1: val('p1') || null,
    prioridade_2: val('p2') || null,
    prioridade_3: val('p3') || null,
    ponto_forte: val('forte') || null,
    desenvolver_mais: val('fraco') || null,
    treina_atualmente: val('treina') || null,
    tempo_treino_experiencia: val('tempo') || null,
    nivel_atual: val('nivel') || null,
    frequencia_semanal: val('freq') || null,
    tempo_por_treino: val('tempoTreino') || null,
    dias_preferidos: checked('dias'),
    descanso_preferido: val('descanso') || null,
    exercicios_gosta: val('gosta') || null,
    exercicios_evitar: val('naogosta') || null,
    estilo_treino: val('estilo') || null,
    limite_esforco: val('limite') || null,
    preferencia_equipamentos: val('equip') || null,
    local_treino: val('local') || null,
    equipamentos_indisponiveis: val('indisponivel') || null,
    dor_ou_limitacao: val('dor') || null,
    dor_detalhes: val('dorDetalhe') || null,
    exercicios_desconforto: val('evita') || null,
    recomendacao_profissional: val('medica') || null,
    recomendacao_detalhes: val('medicaDetalhe') || null,
    treino_personalizado: val('feito') || null,
    item_que_nao_pode_faltar: val('naofalta') || null,
    resultado_desejado: val('frase') || null,
    origem: 'github_pages'
  };
}

function buildSummary(){
  const r=checked('regioes').join(', ');
  const d=checked('dias').join(', ');
  return `ANAMNESE DE TREINO DA ANDRESSA\n\n1. OBJETIVO\nObjetivo: ${empty(val('objetivo'))}\nO que espera do treino: ${empty(val('mudanca'))}\n\n2. PRIORIDADES\nRegiões: ${empty(r)}\n1ª prioridade: ${empty(val('p1'))}\n2ª prioridade: ${empty(val('p2'))}\n3ª prioridade: ${empty(val('p3'))}\nPonto forte: ${empty(val('forte'))}\nDesenvolver mais: ${empty(val('fraco'))}\n\n3. EXPERIÊNCIA\nTreina atualmente: ${empty(val('treina'))}\nTempo de treino: ${empty(val('tempo'))}\nNível: ${empty(val('nivel'))}\n\n4. ROTINA\nFrequência: ${empty(val('freq'))}\nTempo por treino: ${empty(val('tempoTreino'))}\nMelhores dias: ${empty(d)}\nDescanso preferido: ${empty(val('descanso'))}\n\n5. PREFERÊNCIAS\nExercícios de que gosta: ${empty(val('gosta'))}\nExercícios que evita ou não gosta: ${empty(val('naogosta'))}\nEstilo de treino: ${empty(val('estilo'))}\nTreinar próximo do limite: ${empty(val('limite'))}\nEquipamentos: ${empty(val('equip'))}\n\n6. ESTRUTURA\nLocal: ${empty(val('local'))}\nEquipamentos indisponíveis: ${empty(val('indisponivel'))}\n\n7. CONFORTO E LIMITAÇÕES\nDor ou limitação: ${empty(val('dor'))}\nDetalhes: ${empty(val('dorDetalhe'))}\nExercícios evitados: ${empty(val('evita'))}\nRecomendação médica ou profissional: ${empty(val('medica'))}\nDetalhes: ${empty(val('medicaDetalhe'))}\n\n8. TREINO IDEAL\nO que faria o treino parecer feito para você: ${empty(val('feito'))}\nO que gostaria de encontrar no treino: ${empty(val('naofalta'))}\nResultado desejado: ${empty(val('frase'))}\n`;
}

async function sendToSupabase(){
  const response=await fetch(`${SUPABASE_URL}/rest/v1/andressa_anamnese`,{
    method:'POST',
    headers:{
...
finishBtn.onclick=async()=>{
  ...
  finishBtn.innerHTML='Enviando...';
  try{
    await sendToSupabase();
    summary.textContent=buildSummary();
    out.querySelector('h2').textContent='Respostas enviadas com sucesso ♡';
    out.querySelector('.modal-copy').textContent='Suas respostas foram salvas com segurança. Você ainda pode copiar ou compartilhar este resumo.';
    localStorage.removeItem(KEY);
    finishBtn.innerHTML='Respostas enviadas ✓';
...
document.getElementById('clear').onclick=()=>{if(!confirm('Deseja limpar todas as respostas?'))return;localStorage.removeItem(KEY);form.reset();finishBtn.disabled=false;finishBtn.innerHTML='Enviar respostas <span>→</span>';toggle();progress();showToast('Respostas limpas.')};
